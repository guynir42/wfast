...
cal.loadByDate('2020-06-28', 'Balor', 'WFAST'); 

%% go to the "fast mode" data at 30ms exposures

filename1 = fullfile(getenv('DATA'), 'WFAST/examples/SGR1935_magnitudes/WFAST_Balor_20200628-234017-911_F505W_0_Full.h5z');
...
IC1 = cal.input(single(sum(I1,3)), 'sum', 100); 
...
fr = util.fit.polyfit(cat1.data.Mag_BP, log10(cat1.data.snr), 'order', 1, 'double', 1, 'sigma', 2.5, 'iterations', 10);
...
filename2 = fullfile(getenv('DATA'), 'WFAST/examples/SGR1935_magnitudes/WFAST_Balor_20200628-233901-880_F505W_0_Image.h5z');
...
fr = util.fit.polyfit(cat2.data.Mag_BP, log10(abs(cat2.data.snr)), 'order', 1, 'double', 1, 'sigma', 2.5, 'iterations', 10);
util.fit.plot_fit(fr, 'marker', 'p', 'ax', ax2, 'LineWidth', 3); 
