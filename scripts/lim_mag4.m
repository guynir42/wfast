...
% exposure times with the new camera. This includes the new 10Hz mode! 
...
cal.loadByDate('2021-03-07', 'Balor', 'WFAST'); 

base_dir = fullfile(getenv('DATA'), 'WFAST/saved/limmag2');

%% go to the "fast mode" data at 40ms exposures

dir1 = util.sys.WorkingDirectory(fullfile(base_dir, 'limmag_25Hz'));
...
fprintf('Found %d stars with background mean/std of %4.2f / %4.2f\n', height(T1), util.stat.median2(M1), sqrt(util.stat.median2(V1))); 

cutouts1 = util.img.mexCutout(IC1, T1.pos(1:100,:), 15);

width1 = [];
for ii = 1:size(cutouts1, 4)
    width1(ii,1) = util.img.fwhm(cutouts1(:,:,1,ii), 'method', 'filters', 'defocus', 1, 'generalized', 5); 
end

fprintf('Median width is %4.2f"\n', nanmedian(width1).*2.29); 
...
util.sys.print(fullfile(getenv('WFAST'), 'scripts/plots/lim_mag_40ms'));

%% now let's look at some data from the "slow mode"

dir2 = util.sys.WorkingDirectory(fullfile(base_dir, 'limmag_3s'));
...
fprintf('Found %d stars with background mean/std of %4.2f / %4.2f\n', height(T2), util.stat.median2(M2), sqrt(util.stat.median2(V2))); 
...
dir3 = util.sys.WorkingDirectory(fullfile(getenv('DATA'), 'WFAST/saved/limmag/limmag_30s'));
filename3 = dir3.match('*.h5*'); 
filename3 = filename3{5}; 
...
fprintf('Found %d stars with background mean/std of %4.2f / %4.2f\n', height(T3), util.stat.median2(M3), sqrt(util.stat.median2(V3))); 
